...
​	使用正则解析template中的vue的指令（v-xxx）变量等等，形成语法树AST
...
- 你在`Vue`生命周期的created()钩子函数进行的`DOM`操作一定要放在`Vue.nextTick()`的回调函数中。原因是什么呢，原因是在`created()`钩子函数执行的时候`DOM` 其实并未进行任何渲染，而此时进行`DOM`操作无异于徒劳，所以此处一定要将`DOM`操作的`js`代码放进`Vue.nextTick()`的回调函数中。与之对应的就是mounted钩子函数，因为该钩子函数执行时所有的`DOM`挂载和渲染都已完成，此时在该钩子函数中进行任何`DOM`操作都不会有问题 。
...
1、$route对象
![img](https://img2018.cnblogs.com/blog/1288536/201811/1288536-20181115132315183-218072593.png)
...
- 组件内的 this.route和route watcher 回调（监测变化处理）;
...
2、$router对象
![img](https://img2018.cnblogs.com/blog/1288536/201811/1288536-20181115132345300-325891607.png)
...
this.$route.query.name和 this.$route.params.name。
 2.url地址显示
...
[vue.js 使用中踩过的坑](https://segmentfault.com/a/1190000013008420 )
...
[从 Mixin 到 HOC 再到 Hook](https://mp.weixin.qq.com/s/e6jjP1TXq6qFKVn1RtL8pA)
...
### from memory cache与from disk cache区别
...
[CDN作用和基本过程](https://blog.csdn.net/lihao21/article/details/52808747 )
...
1. 浏览器先查看浏览器缓存-系统缓存-路由器缓存。

2. DNS 解析

3. TCP 三次握手

4. 发送请求，分析 url，设置请求报文(头，主体)
